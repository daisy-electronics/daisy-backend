...
const port = new (require('serialport'))('/dev/ttyUSB0')
const parser = port.pipe( new (require('@serialport/parser-readline'))({ delimiter: '\r\n' }));
port.on('data', buf => { [...buf].forEach(byte => console.log("Bits", fromNumber(byte))); });
//port.on('data', buf => console.log("Buf", buf));
